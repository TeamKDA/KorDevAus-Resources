...
Describe "SQL Server Deployment Tests" {
    # Init
    BeforeAll {
        if ($IsLocal -eq $false) {
    ...
    Context "When SQL Server deployed with parameters" {
        ...
        $output = az group deployment validate `
            -g $ResourceGroupName `
            --template-file $SrcDirectory\SqlServer.json `
        ...
        It "Should be the expected name" {
            $expected = "sqlsvr-$organisationCode-$environmentCode-$locationCode"
