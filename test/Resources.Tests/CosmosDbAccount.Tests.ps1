...
Describe "Cosmos DB Account Deployment Tests" {
    # Init
    BeforeAll {
        if ($IsLocal -eq $false) {
    ...
    Context "When Cosmos DB Account deployed with parameters" {
        $organisationCode = "kda"
        $environmentCode = "lcl"
        $locationCode = "ase"
        $output = az group deployment validate `
            -g $ResourceGroupName `
            --template-file $SrcDirectory\CosmosDbAccount.json `
        ...
        It "Should be the expected name" {
            $expected = "cosdba-$organisationCode-$environmentCode-$locationCode"
