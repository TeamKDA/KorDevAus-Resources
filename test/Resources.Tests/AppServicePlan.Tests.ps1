...
Describe "App Service Plan Deployment Tests" {
    # Init
    BeforeAll {
        if ($IsLocal -eq $false) {
    ...
    Context "When App Service Plan deployed with parameters" {
        $organisationCode = "kda"
        $environmentCode = "lcl"
        $locationCode = "ase"
        $output = az group deployment validate `
            -g $ResourceGroupName `
            --template-file $SrcDirectory\AppServicePlan.json `
        ...
        It "Should be the expected name" {
            $expected = "asplan-$organisationCode-$environmentCode-$locationCode"
